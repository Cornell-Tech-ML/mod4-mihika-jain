...
def test_max(t: Tensor) -> None:
    ...
    for batch in range(2):
        for pos in range(4):
            ...
            assert_close(output[batch, 0, pos], expected)
    ...
    for channel in range(3):
        for pos in range(4):
            expected = max([t[i, channel, pos] for i in range(2)])
            assert_close(output[0, channel, pos], expected)
